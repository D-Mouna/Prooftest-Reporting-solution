#!/usr/bin/env python3
"""
OPC Classic DA client for HIMA X-OPC DA instance X_OPC-25138.
...
DCOM (only if connect fails with "Access is denied" / 0x80070005):
    - Run as the same Windows user that can connect in Softing OPC Toolbox.
    - Or use dcomcnfg -> DCOM Config -> HIMA X-OPC / OPC server -> Security.

Usage:
    python Connection-opc.py --list-only
    python Connection-opc.py --tag 200S2503-I11_IN
    python Connection-opc.py --tag "OTS MIRO_T2_1.200S2503-I11_IN"
    python Connection-opc.py --discover-only
"""
