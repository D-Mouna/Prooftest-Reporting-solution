...
| **Filename** | `SPEC-001-v1.56-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.55 |
...
**This version only** — relative to **v1.55**.  
Full history: [History of Modifications.md](./History%20of%20Modifications.md).

**Supersedes v1.55.** Active code: `HIMA-Prooftest-Solution-Current`.
...
| Aspect | Behaviour |
|--------|-----------|
| **Device list** | SILworX REST API **only if the user has a project open**; otherwise scan **X-OPC** (`opc_fallback`). The tool **never** opens a SILworX project. |
...
| **G-10** | To identify the device list, device types, and updates, the solution must use the **SILworX integrated API only when the user has a project open** in SILworX (attach to that session; details per Step below). The solution **must never open a SILworX project** (`POST /project/open/local` is forbidden for the report tool). **If no project is open**, or SILworX/API is closed, suspended, unreachable, or uninstalled, the solution **must update the device list by scanning X-OPC servers** for Prooftest Results devices (`opc_fallback`). Realtime values are always read from **X-OPC**. |
...
| Check | Result |
|-------|--------|
| SILworX installed / API reachable | Device list via **API**; OPC for realtime (and OPC poll if API later goes down) |
| SILworX **not** installed or API unavailable | Device list via **OPC scan** (`opc_fallback`) — same mode, not a separate Case 2 |
...
**Service state:** Persist `device_list_source` in `dbo.ServiceState` as `api` or `opc_fallback` after each device-list sync. Persist `deployment_case = 1`.

#### 3.1 Primary path — SILworX API (+ OPC enrichment)
...
**Fallback path — no user-open project / API unavailable:**

When no SILworX project is open, or the API session cannot be obtained or validated:
...
The report tool **must not** call `POST /project/open/local`. If the user has **no** project open, skip API device discovery and use **OPC scan**.
...
#### 3.2 OPC device-list path (when API unavailable)

Used whenever SILworX/API is not available (HMI station, closed SILworX, G-19 suspend, G-11 uninstall) — **still unified mode**, not a separate Case 2:
...
2. On structural match, add to Device Prooftest Result List:
   ...
   - **`Configuration` / `Resource`** remain **NULL** (OPC path has no API metadata).
3. **Poll every 2 seconds** (`device_list_poll_sec`) while API is unavailable: add new devices, deactivate removed ones. Set `device_list_source = opc_fallback`.
...
| **New / changed SILworX globals** (device add) | Detected indirectly: session / project / codegen / download / manual refresh → API re-read | Step 3 (device list). **Not** by editing Results Structure CSVs |
...
On any trigger: `service.refresh()` calls `sync_device_list_case1_via_api()` — REST API **only if a user-open project can be attached**; otherwise OPC (`opc_fallback`).
...
When the engineer **closes the project** but SILworX remains open, the device list uses **OPC** until a project is open again. G-19 still does **not** kill SILworX merely because the project was closed.
...
| # | Component | Behavior |
|---|-----------|----------|
| **1** | **Device list** (scrolling) | Two exclusive views: **All devices (including deleted with reports)** and **OPC devices only (Running bit)**. Default is all devices. Each row shows whether the device is currently on OPC. Reload on page load and with the health poll. **Search field** (`#device-search`): matches tag or Results type text — highlights matches; **does not hide** other rows. **Enter** cycles to the next match. **Archive lists** writes `devices.csv` + `reports.csv` (and report file copies) under `C:\HIMA Prooftest Reporting Tool\List Archives\`. **Keep OPC devices only** clears non-OPC rows (confirm; optional archive first). **Restore archive** reloads a selected archive into the device list |
...
| **5** | **Alarm / Error** zone | Persistent panel: active alarms, last error, health summary. Health must show **ALL DEVICES** and **OPC ACTIVE DEVICES** next to each other (or stacked). Only **OPC ACTIVE DEVICES** uses the green healthy colour |
...
| **Step 3-C1** | No API device list | User has no SILworX project open | Expected — scan X-OPC (`opc_fallback`). Open the project in SILworX to use API. |
...
4. `service.refresh()` when API unavailable — `device_list_source = opc_fallback`; `deployment_case` state stays **1**.
...
| Unified mode | Always `deployment_case=1`; API only if user has project open, else OPC | `step01_setup.py` + `step03` / `step07` | **Done** (v1.52 — never `open/local`) |
...
| **No project open** | OPC device-list scan (`opc_fallback`) — **required**; the tool never opens a project |
...
|---------|------|--------|-------|
| **1.56** | **2026-08-17** | **Report Solution** | Archive/restore device and report lists; keep-OPC-only clear |
