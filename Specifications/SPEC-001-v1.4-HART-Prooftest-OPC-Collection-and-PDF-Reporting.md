...
| **Filename** | `SPEC-001-v1.4-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.3 |
...
2. On **structural match** (member set / naming pattern), add to Device Prooftest Result List with **`Device_TAG`** = **OPC variable name** (root tag) and **`OPC_Server`** = server where the match was found.
3. **Every 2 seconds**:
   - Re-scan **all** X-OPC servers.
   - Add newly detected devices.
   - Deactivate devices no longer present on any server.

**Matching rule (implementation):** For each server, find all tags ending in `.Running`; treat the parent path as the device prefix; if sibling members match a Results CSV pattern (`Running`, `Error`, …), bind to that Results type. Prefer the server with the highest member match score when duplicates exist.
