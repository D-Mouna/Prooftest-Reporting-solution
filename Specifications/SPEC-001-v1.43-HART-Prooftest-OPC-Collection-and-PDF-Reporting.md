...
| **Filename** | `SPEC-001-v1.43-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.42 |
...
**This version only** — modifications relative to the superseded version (**v1.42**).  
...
|-------|-------|----------------|
| **SQL table creation** | Spec/code oriented toward reading `.sql` files from `2- SQL Tables template` at runtime | Runtime **generates** `ProofTest_*` tables from **Results Structure** definitions (CSV), matching the column style of the project templates |
| **Template folder** | Appeared as a station dependency (`sql_templates` path) | **Design / development reference only** (`C:\` or `Z:\Project\Report Solution\2- SQL Tables template`). **Not required** for a first run of the Report Solution on another PC |
| **Bundled definitions** | Results CSVs often pointed at the engineering share only | Solution ships `Results Structures\` (nine CSVs); `results_structures` defaults to that folder |

#### Description of v1.43 modifications

1. Clarified G-05 / Step 1.3 / Step 2: DDL is code-generated; template directory is not a runtime dependency.
2. Case 2 schema updates follow **Results Structure** changes, not new `.sql` template files on disk.
3. Implementation creates tables via CSV-derived DDL (template-compatible types); optional `.sql` apply only if a template file happens to be present.

#### Files touched for v1.43

| File | Change |
|------|--------|
| `SPEC-001-v1.43-...` | Runtime vs design-reference wording |
| `Annex codes/Database/annex_database.py` | Primary DDL from Results structures |
| `Results Structures\*.csv` | Bundled with Current |
| `Tool Steps/config.py` / `solution.ini` | Default `results_structures`; `sql_templates` optional |
| `VERSION.json` | 1.43 |
...
|---------|------|--------|-------|
| **1.43** | **2026-08-12** | **Report Solution** | Runtime creates nine `ProofTest_*` tables from Results Structure CSVs (template-style DDL). `2- SQL Tables template` is design reference only — not required on other PCs |
