...
| **Filename** | `SPEC-001-v1.30-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.29 |
...
Cumulative record of specification and implementation updates. **The current version (1.30) is listed first**; summaries from all prior gated releases are retained below for audit. Active code: `HIMA-Prooftest-Solution-v1.30`.

### Version 1.30 (2026-06-19) — current

**Supersedes v1.29.** Active code folder: `HIMA-Prooftest-Solution-v1.30`. Prior folder `HIMA-Prooftest-Solution-v1.29` is **frozen**.

#### What changed from v1.29 to v1.30

| Topic | v1.29 | v1.30 (change) |
|-------|-------|----------------|
| **Hero key visual** | Static plant photo background | Split hero banner: branding/actions on the left; looping **`HIMA_animation_sgd graph and key visual.mp4`** on the right (`Graphic Interface/static/img/hima-animation-sgd.mp4`) |
| **Accessibility** | — | Hero video pauses when `prefers-reduced-motion: reduce` |

#### Files touched for v1.30

| File | Change |
|------|--------|
| `Graphic Interface/static/index.html` | Hero layout with `<video id="hero-key-visual">` |
| `Graphic Interface/static/style.css` | `.hero-layout`, `.hero-brand`, `.hero-visual` |
| `Graphic Interface/static/app.js` | Reduced-motion handling for hero video |
| `Graphic Interface/static/img/hima-animation-sgd.mp4` | Copied from `7- Images for the graphical interface` |
| `SPEC-001-v1.30-...md` | §5.1 visual design updated |
...
| **Gates 10–13** | Scope defined; not started | **Gate 10 approved** 2026-06-18; **Gate 11 in progress**; Gates 12–13 pending |
...
| **11** | Web UI + alarms (`test_step11_web_ui.py`) | **In progress** — polish + gate test |
| **12** | Case 2 deployment | **Code done** — validation pending |
| **13** | Hardening (`/api/health`, auth, full column mapping) | **Not started** |
...
**Scroll list placeholders:** The device list and report list panels (§5.1 #1–2) must **always** show a bordered scrolling area. When no devices are detected in the Device Prooftest Result List, the device list displays **`(No device selected)`**. When no report is available or selected, the report list displays **`(No report selected)`**. Lists remain visible even when empty.

**Visual design:** The web GUI follows the HIMA proof-test report layout (split hero with HIMA logo, HART/SIL badges, Verdana typography). The hero banner plays the **`#Safetygoesdigital`** key visual (`HIMA_animation_sgd graph and key visual.mp4`, served as `img/hima-animation-sgd.mp4`) beside branding and service controls. Branding assets are copied from `Z:\Project\Report Solution\7- Images for the graphical interface` into `Graphic Interface/static/img/` (HIMA, HART, SIL, and manufacturer logos). Device list rows show the vendor logo mapped from `Results_Type`; footer displays supported manufacturer logos.
...
**Known issue (gate 2):** `GET /api/health` may block when OPC is busy; hardening deferred to Gate 13.
...
```powershell
cd "Z:\Project\Report Solution\Codes\HIMA-Prooftest-Solution-v1.16"
.\stop_service.ps1
```
...
- [ ] Full column mapping OPC/CSV → SQL placeholder aliases for all nine types (ongoing as templates evolve).
...
- [ ] Gate 11 — Web UI + alarms gate test (`test_step11_web_ui.py`): API routes, `AlarmLog` persistence, health summary.
...
| Item | Path |
|------|------|
| Solution code (active, v1.30) | `Z:\Project\Report Solution\Codes\HIMA-Prooftest-Solution-v1.30\` |
...
| **1.30** | **2026-06-19** | **Report Solution** | **Web GUI hero:** `#Safetygoesdigital` animation key visual in split hero banner; active code `HIMA-Prooftest-Solution-v1.30` |
