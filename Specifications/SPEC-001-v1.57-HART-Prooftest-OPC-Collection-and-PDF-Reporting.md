...
| **Filename** | `SPEC-001-v1.57-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.56 |
...
**This version only** — relative to **v1.56**.  
Full history: [History of Modifications.md](./History%20of%20Modifications.md).

**Supersedes v1.56.** Active code: `HIMA-Prooftest-Solution-Current`.
...
| # | Component | Behavior |
|---|-----------|----------|
| **1** | **Device list** (scrolling) | Two exclusive views: **All devices (including deleted with reports)** and **OPC devices only (Running bit)**. Search and the archive-before-clear checkbox sit **bottom-right**, immediately above the list; the checkbox has no white fill. **Archive lists** and **Clear device list** are action buttons (hover on Clear: “Keep OPC devices only”). After Archive, show the archive folder path at the **bottom of the page**. Restore includes **Browse** to upload `devices.csv` or a zip. |
...
| **5** | **Alarm / Error** zone | Aligned with Service health, Device list, and Report list in a **2×2** grid. Each alarm shows **still active** or **no longer exists**. Operator can **acknowledge** an alarm and **reset** the list. Health card order: Service + Database; then Device list, SILworX session, Plugin session, Queue depth; last ALL DEVICES and OPC ACTIVE DEVICES (OPC green only) |
...
|---------|------|--------|-------|
| **1.57** | **2026-08-17** | **Report Solution** | 2×2 panel layout; health card order; archive path; browse restore; alarm ack/reset |
