...
| **Filename** | `SPEC-001-v1.46-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.45 |
...
**This version only** — modifications relative to the superseded version (**v1.45**).  
Full history of all versions: [History of Modifications.md](./History%20of%20Modifications.md).

**Supersedes v1.45.** Active code: `HIMA-Prooftest-Solution-Current`.

#### What changed from v1.45 to v1.46

| Topic | v1.45 | v1.46 (change) |
|-------|-------|----------------|
| **Station root** | Reports / CSVs / DB split across locations | Unified under **`C:\HIMA Prooftest Reporting Tool`** |
| **Layout** | — | `Results Structures\`, `HIMA Automated Prooftest Reports\`, `Database\` |
| **New CSV** | Type catalogue | New `*.csv` in station Results Structures = new Results / device type |

#### Description of v1.46 modifications

1. All station runtime data lives under `C:\HIMA Prooftest Reporting Tool`.
2. SQL Server DB name remains `HIMA Automated Prooftest`; SQLite file is `...\Database\prooftest.db`.
3. First start creates the root, seeds CSVs from the package, and migrates legacy C: folders when present.
...
| **G-05** | On **first start** on a station: (1) create **`C:\HIMA Prooftest Reporting Tool`** with subfolders `HIMA Automated Prooftest Reports`, `Results Structures`, and `Database` (Step 1); (2) create SQL database **`HIMA Automated Prooftest`** if missing (SQLite fallback file under `...\Database\prooftest.db`); (3) create a `ProofTest_*` table for **each** Results Structure CSV in the station catalogue (baseline nine plus any added types). Table DDL is **generated by the solution** from those CSVs (style aligned with project `2- SQL Tables template` — **design reference only**). Deployed stations do **not** require the template folder. |
...
|---------|------|--------|-------|
| **1.46** | **2026-08-14** | **Report Solution** | Station root `C:\HIMA Prooftest Reporting Tool` (Reports, Results Structures, Database) |
