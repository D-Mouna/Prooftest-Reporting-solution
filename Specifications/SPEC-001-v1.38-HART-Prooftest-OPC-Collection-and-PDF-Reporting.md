...
| **Filename** | `SPEC-001-v1.38-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.33 |
...
**This version only** — modifications relative to the superseded version (**v1.33**).  
Full history of all versions: [History of Modifications.md](./History%20of%20Modifications.md).

**Supersedes v1.33.** Active code: `HIMA-Prooftest-Solution-Current` (prior tree archived as `HIMA-Prooftest-Solution-v1.37`).
...
#### Description of v1.38 modifications

1. **Engine stop vs process exit (§5.1 #7, §5.5).** UI **Stop service** calls `POST /api/stop` and releases OPC / SILworX API / plugin monitors without exiting the Python process, so the graphic interface remains usable. UI **Start service** calls `POST /api/start` to restart the engine in-process.
2. **Uninstall path unchanged (G-11).** `stop_service.ps1` and `POST /api/shutdown` still exit the process for SILworX uninstall.
3. **Health while stopped.** `/api/health` keeps answering with `engine_running: false` and `web_host_alive: true`.
...
| `SPEC-001-v1.38-...md` | §5.1 #7, §5.4, §5.5 |
| `History of Modifications.md` | Cumulative change log (all versions) |
...
|---------|------|--------|-------|
| **1.38** | **2026-08-06** | **Report Solution** | **Engine stop vs process exit:** UI Stop keeps graphic interface alive (`POST /api/stop`); Start restarts engine in-process; G-11 uninstall still `POST /api/shutdown` / `stop_service.ps1`; active code `HIMA-Prooftest-Solution-Current` |
