...
| **Filename** | `SPEC-001-v1.21-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.20 |
...
| **G-20** | **Kill hung SILworX Windows background processes after confirmed close:** when SILworX was previously running in this service session, the API is unreachable for **four consecutive** background polls (~8 s), **no project is open** (`lock.ini` absent), and leftover processes remain (`c3.exe`, `OLixClient.exe`, `hima.*` plugins), terminate them via `annex_silworx_cleanup.py` (`taskkill /F /T`). **Never** kill while SILworX API responds, a project is open, or SILworX is starting up (API-only `c3.exe` is normal). Record killed-count in `ServiceState.silworx_cleanup_killed`. |
...
| **4th+ consecutive failure, no project open, seen running before** | `kill_hung_silworx_processes()` — terminate leftover `c3.exe` / plugin processes only after confirmed SILworX close (G-20); **never** while API responds or project is open |
...
sqlite_path = Z:\Project\Report Solution\Codes\HIMA-Prooftest-Solution-v1.21\Annex codes\data\prooftest.db
...
| Item | Path |
|------|------|
| Solution code (active, v1.21) | `Z:\Project\Report Solution\Codes\HIMA-Prooftest-Solution-v1.21\` |
