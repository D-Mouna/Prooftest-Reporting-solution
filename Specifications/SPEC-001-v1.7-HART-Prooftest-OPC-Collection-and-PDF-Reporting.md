...
| **Filename** | `SPEC-001-v1.7-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.6 |
...
| Global variables export changed | `globals_export` CSV mtime increased | `globals_export` |
...
> **Note:** Device list (Part 2.1) still reads `globals_export` CSV. Session watch detects project saves; export globals to `globals_export` after editing (or use manual Refresh after export) so device types are current.
...
sync_triggers = silworx_session, code_generation, download, globals_export, results_structures
