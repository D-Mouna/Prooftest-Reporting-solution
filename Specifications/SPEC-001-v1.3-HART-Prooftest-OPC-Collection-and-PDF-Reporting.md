...
| **Filename** | `SPEC-001-v1.3-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.2 |
...
| Trigger | Detection |
|---------|-----------|
| Code generation completed | Watch SILworX output / log / timestamp |
| Project downloaded | Watch `.E3` / deploy folder modification |
| New CSV in Results Structures | File watcher |
| Manual | Web GUI **Refresh / Reset** or `ForceSchemaSync=1` |
...
1. Read the **global variable list** of the SILworX application program.
2. Identify variables whose **data type** is a Results structure (§4).
3. Set **`Device_TAG`** = global variable **name** (e.g. `100-FZT-001`).
4. On each **code generation + download** (and on manual refresh):
   - **Add** new devices.
...
1. Compare **realtime data** on all running X-OPC servers to each Results structure defined in the CSV files (§4).
2. On **structural match** (member set / naming pattern), add to Device Prooftest Result List with **`Device_TAG`** = **OPC variable name** (root tag).
3. **Every 2 seconds**:
   - Add newly detected devices.
   - Deactivate devices no longer present in OPC.

**Matching rule (implementation):** For each OPC tag tree branch, if leaf members match a Results CSV member naming pattern (`<prefix>.Running`, `<prefix>.Error`, …), bind to that Results type.
...
2. The solution **searches all X-OPC servers running on the host** for each device's members.

| Step | Action |
|------|--------|
| 1 | Enumerate OPC DA servers (`opc.servers()` / OpcEnum) |
| 2 | Filter to X-OPC / HIMA (config: `*X_OPC*`, `*HIMA*`) |
| 3 | Connect to each running server |
| 4 | For each active device, resolve `Device_TAG` → `(OPC_Server, ItemPrefix)` across all servers |
| 5 | Read **leaf** OPC items only; cache mapping; refresh on failed read |
...
- [ ] Confirm SILworX **code generation / download** watch path per project layout (Case 1).
