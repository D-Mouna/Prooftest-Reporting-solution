...
| **Filename** | `SPEC-001-v1.39-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.38 |

> **Versioning:** Updates require a new file (e.g. `SPEC-001-v1.15-...`). Each SPEC file’s **Summary of changes** covers **only** the delta from the previous version; the full log is [History of Modifications.md](./History%20of%20Modifications.md). See [README.md](./README.md). Code changes: [Codes/README.md](../Codes/README.md).

---

## Summary of changes

**This version only** — modifications relative to the superseded version (**v1.38**).  
Full history of all versions: [History of Modifications.md](./History%20of%20Modifications.md).

**Supersedes v1.38.** Active code: `HIMA-Prooftest-Solution-Current` (prior tree archived as `HIMA-Prooftest-Solution-v1.38`).

#### What changed from v1.38 to v1.39

| Topic | v1.38 | v1.39 (change) |
|-------|-------|----------------|
| **UI Start after Stop** | Engine start could hang UI: `/api/health` blocked on OPC lock during refresh; SILworX API stayed **suspended** after Stop | Start clears API suspend; health returns fast `starting` without OPC; OPC refresh runs without holding the engine lock |
| **UI feedback** | Start returned immediately; UI polled health (timed out) and looked dead | UI waits/polls until `engine_running` (up to ~3 min) with “starting” banner |

#### Description of v1.39 modifications

1. **`prepare_for_engine_start()`** clears G-19 `_silworx_api_suspended` so Case 1 API discovery works again after Stop.
2. **`_starting` health path** — `/api/health` does not call OPC while the engine is starting.
3. **Start no longer holds `_engine_lock` during OPC browse** — Stop/health remain responsive.
4. **Graphic interface** polls until the engine is running after Start.

#### Files touched for v1.39

| File | Change |
|------|--------|
| `Tool Steps/service.py` | `_starting`; unlock during start body; health starting path |
...
|---------|------|--------|-------|
| **1.39** | **2026-08-12** | **Report Solution** | Fix UI Start after Stop: clear API suspend; non-blocking health while starting; unlock during OPC refresh |
