...
| **Filename** | `SPEC-001-v1.19-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.17 |
...
| **G-20** | **Kill hung SILworX Windows background processes:** when SILworX is closed or enters a “hung background” state, the service must force-terminate SILworX Windows processes (`c3.exe`, `OLixClient.exe`, `hima.plugin_manager.exe`, `hima.ssl_certificates_creator.exe`) using `annex_silworx_cleanup.py` (`taskkill /F /T` from CIM/WMI discovery). If the API is already down, cleanup runs after the G-19 down-streak; if the SILworX GUI/helper layer is gone but the API/plugin processes still exist, cleanup runs immediately. Record killed-count in `ServiceState.silworx_cleanup_killed`. |
...
| **6** | Case 1 device list via API globals + OPC fallback; persist `device_list_source` | §4 Step 3.1 | **Code done** — API path pending valid session on target station (`Tool test/test_step6_devices.py`) |
...
2. Obtain session via SILworX **plugin WebSocket** `TRIGGER_SESSION_ID_CHANGED` (`API connexion/annex_api_connexion.py` + `Plugin/annex_plugin.py`).
...
| **2nd+ consecutive failure** | `release_api_connection()` — discard client, clear session ids, set `silworx_api_connected = 0`, set `is_api_suspended` so `api_session` / device-list refresh will **not** call `open/local` until SILworX responds again; then run one-shot hung-process cleanup (`annex_silworx_cleanup.kill_hung_silworx_processes`) and write `silworx_cleanup_killed` |
| **GUI/helper layer gone, API/plugin still present** | Immediate hung-process cleanup when non-API GUI helpers (`OLixClient.exe` and non-API `c3.exe`) are gone but API/plugin-related processes still exist (`c3.exe` API server and `hima.*` helpers) |
...
HIMA-Prooftest-Solution-v1.17/
  ...
  Stop service/
    ...
    annex_silworx_cleanup.py   ← kill stale SILworX helper processes (G-20)
...
sqlite_path = Z:\Project\Report Solution\Codes\HIMA-Prooftest-Solution-v1.19\Annex codes\data\prooftest.db
...
| Item | Path |
|------|------|
| Solution code (active, v1.19) | `Z:\Project\Report Solution\Codes\HIMA-Prooftest-Solution-v1.19\` |
...
| **1.19** | **2026-06-16** | **Report Solution** | **G-19:** release SILworX API connection when SILworX software is closed (`is_silworx_running` via `POST /silworx/info`, `release_api_connection` in `step07_triggers.py`); active code `HIMA-Prooftest-Solution-v1.19`. **G-19 fix:** persistent `open/local` session ownership (`_service_owns_api_session`), consecutive-probe release, `try_close_owned_session` on first failure, `is_api_suspended` blocks re-open while SILworX down; `solution.ini` `api_port=51711` / `api_plugin_port=8401` for GUI instance |
| **1.19** | **2026-06-16** | **Report Solution** | **G-20 addition:** dedicated SILworX hung-process cleanup module (`annex_silworx_cleanup.py`) kills stale `c3.exe` / `hima.*` helper processes after SILworX close is confirmed; one-shot per outage, writes `ServiceState.silworx_cleanup_killed` |
