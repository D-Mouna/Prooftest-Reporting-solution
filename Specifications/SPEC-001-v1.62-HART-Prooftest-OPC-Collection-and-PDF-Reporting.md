...
| **Filename** | `SPEC-001-v1.62-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.61 |
...
**This version only** — relative to **v1.61**.  
Full history: [History of Modifications.md](./History%20of%20Modifications.md).

**Supersedes v1.61.** Active code: `HIMA-Prooftest-Solution-Current` (code 1.65).

#### What changed to v1.62

| Topic | Change |
|-------|--------|
| **Report folders** | Reports are stored per Results type, `Project` (from DeviceId) and `Device_TAG`: `<Results_Type>/<Project>/<Device_TAG>/`. Legacy tag-only folders are still readable. |
| **`/api/reports` lookup** | `GET /api/reports` accepts optional `project` and/or `device_id` query params so same `Device_TAG` across projects doesn't mix report lists. |
| **Production poll/report** | Production realtime detection uses `LiveTestService` with a DeviceId-keyed `RunningEdgeDetector`; snapshot SQL insert + report generation happen via the completion path. |
| **Presentation/controllers** | Web routes live in `Annex codes/layers/presentation/controllers.py` and call application/query services only (no OPC/SILworX logic in presentation). |
...
| 2 | `HIMA Automated Prooftest Reports\` | Generated PDF/HTML prooftest reports (per Results type / Project / Device_TAG). Subfolder `Report Templates\` holds HTML report templates |
...
C:\HIMA Prooftest Reporting Tool\HIMA Automated Prooftest Reports\
  X-HART_ABB_FCB400_Results\
    <Project>\             ← one subfolder per project (from DeviceId)
...
On Windows, replace `/` in Results type folder names with `-` (e.g. `X-HART_E+H_Promass300-500_Results`). Sanitize invalid path characters in `Project` and `Device_TAG` when creating subfolders.

*Example:* Inside `X-HART_WIKA_T32_Results\`, create `<Project>/200-TT-1001\` for each WIKA T32 device in the Device Prooftest Result List.
...
| `SilworxProject` | SILworX project name where the device was detected via API (NULL if never seen on API) |
...
- API wins `Results_Type`, `Configuration`, `Resource`, and `SilworxProject`.
...
- **Per-device source:** if `PresentOnOpc = 1`, source is the **OPC server ProgID**; otherwise source is the **SILworX project** stored in `SilworxProject`.
...
3. **Immediately after** the INSERT completes, generate PDF/HTML (Step 6) and store under:
   ...
   ```text
   Z:\Project\Report Solution\Reports\<Results_Type>\<Project>\<Device_TAG>\
   ```

   and mirror to:

   ```text
   C:\HIMA Prooftest Reporting Tool\HIMA Automated Prooftest Reports\<Results_Type>\<Project>\<Device_TAG>\
   ```
...
| # | Component | Behavior |
|---|-----------|----------|
| **1** | **Device list** (scrolling) | Two exclusive views: **All devices (including deleted with reports)** and **OPC devices only (Running bit)**. Each row shows the **source**: `OPC: {ProgID}` for devices on X-OPC, or `Project: {SILworX project}` for devices not on OPC. Search and the archive-before-clear checkbox sit **bottom-right**, immediately above the list; the checkbox has no white fill. **Archive lists** and **Clear device list** are raised action buttons (hover on Clear: “Keep OPC devices only”). After Archive, show the archive folder path at the **bottom of the page**. Restore is **Browse restore file…** to upload `devices.csv` or a zip. |
...
| `GET /api/reports?device={tag}&project={project}` | Reports scoped by device tag + project (DeviceId) |
...
3. `GET /api/reports?device={tag}&results_type={type}` — scoped lookup under Results-type + `project/` (when `project` is provided).
...
|---------|------|--------|-------|
| **1.62** | **2026-08-19** | **Report Solution** | Report folders scoped by Project/DeviceId to avoid report mixing; presentation controllers split; production poll/report uses LiveTestService completion path |
