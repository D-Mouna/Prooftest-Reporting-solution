...
| **Filename** | `SPEC-001-v1.12-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.11 |

> **Versioning:** Updates require a new file (e.g. `SPEC-001-v1.13-...`). See [README.md](./README.md). Code changes require a new folder per [Codes/README.md](../Codes/README.md).
...
| **3** | `SilworxApiClient` REST wrapper | §4 Step 3.1 | **Done** (`prooftest/silworx_api.py`, `test_silworx_api.py`) |
| **4** | First-run folders (nine Results-type folders + per-device subfolders), `deployment_case` | §4 Step 1 | **Done** (`prooftest/install.py`, `test_step4_install.py`) |
| **5** | All nine `ProofTest_*` SQL tables; generate missing `.sql` from CSV | §4 Step 2 | **Done** (`prooftest/sql_templates.py`, `test_step5_sql.py`) |
...
**Template map (implementation Step 5):** Each of the nine Results types maps to a `.sql` file under `2- SQL Tables template\` via `TEMPLATE_MAP` in `prooftest/sql_templates.py`. If a template file is missing, generate it from the Results Structure CSV before `CREATE TABLE`. SAMSON 3730 and 3793 both use `Prooftest_SAMSON_3793_V1_5.sql` → `ProofTest_SAMSON_Results`.
...
**Not used for Case 1 device list:**

- CSV export of global variables (SILworX plugin or manual export).
...
3. Plugin name in `settings.ini` `[Plugin_Server] Development=` must match the registered development plugin (e.g. `export_globals_plugin`).
...
**Reference implementation:** `5- API Application Example\sapi.py`, `prooftest/silworx_api.py`, `prooftest/silworx_device_list.py`.
...
```powershell
cd "Z:\Project\Report Solution\Codes\HIMA-Prooftest-Solution-v1.12"
.\stop_service.ps1
```
...
|------|------------------|----------------------------|--------|
| Case 1 device list | SILworX API global variables | `silworx_device_list.py` — structuretree + globals read | **Code done** |
...
| First-run folders | 09 type folders + device subfolders | `install.py`; `/` → `-` on disk | **Done** |
| SQL templates (all 9) | Step 2 | `sql_templates.py` + `TEMPLATE_MAP` | **Done** |
...
| Case auto-detect | Auto-detect SILworX in Step 1 | `install.py` + `auto_detect_case` | **Done** |
| Globals CSV file | Not required | Removed; API replaces export | **OK** |
...
- [ ] Production SILworX plugin session bridge deployment (beyond development `export_globals_plugin`).
...
| Item | Path |
|------|------|
| Solution code (active, v1.12) | `Z:\Project\Report Solution\Codes\HIMA-Prooftest-Solution-v1.12\` |
| Solution code (frozen, v1.11) | `Z:\Project\Report Solution\Codes\HIMA-Prooftest-Solution-v1.11\` |
