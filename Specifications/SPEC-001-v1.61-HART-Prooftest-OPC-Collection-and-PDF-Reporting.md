...
| **Filename** | `SPEC-001-v1.61-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.60 |
...
**This version only** — relative to **v1.60**.  
Full history: [History of Modifications.md](./History%20of%20Modifications.md).

**Supersedes v1.60.** Active code: `HIMA-Prooftest-Solution-Current` (code 1.64).

#### What changed to v1.61

| Topic | Change |
|-------|--------|
| **Layers** | Logic is split toward Presentation / Application / Domain / ports. Domain lives in `Annex codes/layers/` (no FastAPI, OpenOPC, or pyodbc). FastAPI routes call the engine/query services — not OPC COM or SILworX HTTP. |
| **Device identity** | Catalog key is **DeviceId = Project + Configuration + Resource + Device_TAG**. The same TAG in two SILworX projects is two rows. Same DeviceId in SILworX and OPC is one row. |
| **OPC bind** | When SILworX already provided the TAG, bind `OTS ProofTest.{TAG}.Running` then `OPC ProofTest.{TAG}.Running`. Do not CSV-score a type for that DeviceId. Same TAG + same OPC path from two DeviceIds → collision alarm S3. |
| **Web devices table** | Columns **Project** and **OPC server**; sort Device_TAG, then Project, then OPC server. `GET /api/devices` returns those fields. |
| **SILworX connect/disconnect** | **Connect to SILworX** / **Disconnect SILworX** drop or resume **this tool’s** API client and plugin monitor only. They never quit SILworX, never `project/close` the engineer’s GUI project, never `taskkill c3.exe`. Engine stays running; catalog falls back to OPC-only while disconnected. |
| **Health** | `engine` starting/running/stopped; `opc_count` (X-OPC servers, not devices); `device_count`; `queue_depth`; `silworx_status` not connected/running; `last_error` `{step, action, message}`. Health does not browse OPC. |
| **Errors** | Failures map to step codes S1–S7 / GUI and `AlarmPort`. Poll/sync/report loops catch per device. PDF failure keeps the SQL snapshot (S6). Reconcile marks inactive; never deletes snapshot history. |
| **Tests** | `Annex codes/Tool test/test_layers.py` — Domain + Application with fake ports (no real OPC/SILworX). |
...
|---------|------|--------|-------|
| **1.61** | **2026-08-18** | **Report Solution** | Layered architecture; composite DeviceId; GUI Project/OPC columns; SILworX attach/detach without killing SILworX |
