...
| **Filename** | `SPEC-001-v1.54-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.53 |
...
**This version only** — relative to **v1.53**.  
Full history: [History of Modifications.md](./History%20of%20Modifications.md).

**Supersedes v1.53.** Active code: `HIMA-Prooftest-Solution-Current`.
...
| # | Component | Behavior |
|---|-----------|----------|
| **1** | **Device list** (scrolling) | All tags currently in **Device Prooftest Result List** (`IsActive = 1`), including newly detected devices and removed devices that still have reports. Reload on page load and with the health poll (do not wait for Refresh; do not hide the list when the engine is stopped). User selects one device. **Search field** (`#device-search`): matches tag or Results type text — highlights matches, scrolls to the current match; **does not hide** other rows. **Enter** cycles to the next match |
...
|---------|------|--------|-------|
| **1.54** | **2026-08-17** | **Report Solution** | Device list: add new devices; delete removed devices only when they have no reports |
