...
| **Filename** | `SPEC-001-v1.55-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.54 |
...
**This version only** — relative to **v1.54**.  
Full history: [History of Modifications.md](./History%20of%20Modifications.md).

**Supersedes v1.54.** Active code: `HIMA-Prooftest-Solution-Current`.
...
| # | Component | Behavior |
|---|-----------|----------|
| **1** | **Device list** (scrolling) | Two exclusive views: **All devices (including deleted with reports)** and **OPC devices only (Running bit)**. Default is all devices. Each row shows whether the device is currently on OPC. Reload on page load and with the health poll. **Search field** (`#device-search`): matches tag or Results type text — highlights matches; **does not hide** other rows. **Enter** cycles to the next match |
...
| **5** | **Alarm / Error** zone | Persistent panel: active alarms, last error, health summary. Health must include **ALL ACTIVE DEVICES** and **ACTIVE DEVICES ON OPC** counts |
...
|---------|------|--------|-------|
| **1.55** | **2026-08-17** | **Report Solution** | Health ALL ACTIVE vs OPC counts; device list all/OPC view options |
