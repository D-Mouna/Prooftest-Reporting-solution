...
| **Filename** | `SPEC-001-v1.6-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.5 |
...
|---------|-----------|------------|
| Code generation completed | `.E3` project file mtime increased (SILworX **not** holding `.E3.lock`) | `sync_triggers` → `code_generation` |
| Project downloaded | Same as above (`.E3` mtime) | `sync_triggers` → `download` |
| Global variables changed | `globals_export` CSV mtime increased (SILworX full export; **independent of `.E3.lock`**) | `sync_triggers` → `globals_export` |
| New CSV in Results Structures | Aggregate mtime of `3- Results Structures\*.csv` | `sync_triggers` → `results_structures` |
| Manual | Web GUI **Refresh / Reset** or `POST /api/refresh` | always |

Background loop polls Case 1 triggers every **`case1_sync_poll_sec`** (default **2 s**). On any trigger: reload structures (if Results CSV changed), rebuild device list (Part 2.1), sync SQL schema (Part 1.1), commit sync markers.
...
## P2.1 Case 1 — From SILworX global variables

1. Read the **global variable list** from `globals_export` (SILworX CSV export) or, if missing, alarm and skip.
2. Consider only **top-level** globals (name has **no** `.` segment) whose **data type** is a known Results structure (§4).
3. Set **`Device_TAG`** = global variable **name** (e.g. `100-FZT-001`).
4. Resolve **`OPC_Server`** / **`OPC_ItemPrefix`** via OPC browse when available (Part 3).
5. On each automatic trigger (§P1.1) or manual refresh:
   - **Add** new devices.
   - **Update** `Results_Type` when the global data type changes.
   - **Deactivate** (`IsActive = 0`) devices no longer in the global list.
   - **Create / update** `ProofTest_*` tables for all active Results types.
...
globals_export = Z:\Project\Report Solution\Globale variable.csv
sync_triggers = code_generation, download, globals_export, results_structures
