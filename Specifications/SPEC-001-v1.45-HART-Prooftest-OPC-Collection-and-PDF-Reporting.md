...
| **Filename** | `SPEC-001-v1.45-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.44 |
...
**This version only** — modifications relative to the superseded version (**v1.44**).  
Full history of all versions: [History of Modifications.md](./History%20of%20Modifications.md).

**Supersedes v1.44.** Active code: `HIMA-Prooftest-Solution-Current`.

#### What changed from v1.44 to v1.45

| Topic | v1.44 | v1.45 (change) |
|-------|-------|----------------|
| **How devices appear** | Could be read as if operators edit Results Structure CSVs | Clarified: the engineer creates **SILworX global variables** whose **type** is one of the nine Results structures; CSVs are the **fixed type catalogue** |
| **CSV role** | Emphasized as runtime DDL source | Same DDL role, but explicitly **not** the day-to-day way to add a device |
| **Step 7 triggers** | “New Results Structures CSV” looked like a normal device-add path | Relabeled as **definition maintenance only**; new devices come from **new globals** via API refresh |

#### Description of v1.45 modifications

1. Strengthened §3.1 / §3.2 wording: globals = instances; CSVs = type definitions.
2. Clarified Step 7 trigger table and Case 1 refresh narrative.
3. Updated architecture docs / Mermaid / diagram 3 accordingly.
4. Code comments/docstrings aligned (no behaviour change).

#### Files touched for v1.45

| File | Change |
|------|--------|
| `SPEC-001-v1.45-...` | Globals vs CSV clarification |
| `HIMA-Prooftest-Functionality-and-Architecture.md` | Same clarification |
| `HIMA-Prooftest-Architecture-Mermaid.md` | Trigger-layer wording |
| `architecture-diagrams/03-three-layers.png` | Regenerated |
| `Tool Steps/results_csv.py`, `step07_triggers.py` | Docstrings |
| `VERSION.json` | 1.45 |
...
| **G-05** | On **first start** on a station: (1) create **`C:\HIMA Automated Prooftest Reports`** (Step 1 structure); (2) create SQL database **`HIMA Automated Prooftest`** if missing; (3) create a table for **each** of the **nine** Results types named per §3.3 (e.g. `ProofTest_E+H_Promass300_500_Results`). Table DDL is **generated by the solution** from the Results Structure definitions (style aligned with project `2- SQL Tables template` — **design reference only**). Deployed stations do **not** require the template folder. |
...
- Alternate / runtime path: solution `Results Structures\` (bundled with Current) or `[Paths] results_structures` in `solution.ini`
...
Every **1 second** (or on Results Structure change triggers), ensure `ProofTest_*` tables exist/match the loaded Results Structure definitions; create or alter tables when definitions change. **Do not** depend on new files appearing under `2- SQL Tables template\`.
...
| **Results Structure CSV definition update** | Folder mtime under `results_structures` / `3- Results Structures` | Step 2 only — **rare type-catalogue maintenance**, not normal device add |
| **Manual refresh** | Web UI **Refresh / Reset** or `POST /api/refresh` | All of Steps 2–4 |

On any Case 1 trigger: `service.refresh()` calls `sync_device_list_case1_via_api()` — **REST API only** for device list and `Results_Type` by reading **global variables** whose type is one of the nine Results structures (OPC fallback only when API unavailable per OI-3).
...
|---------|------|--------|-------|
| **1.45** | **2026-08-14** | **Report Solution** | Clarified: devices = SILworX **global variables** typed as one of nine Results structures; Results Structure CSVs = fixed type catalogue (not operator device-add path) |
