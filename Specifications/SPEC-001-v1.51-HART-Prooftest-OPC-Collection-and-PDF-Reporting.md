...
| **Filename** | `SPEC-001-v1.51-HART-Prooftest-OPC-Collection-and-PDF-Reporting.md` |
| **Supersedes** | v1.50 |
...
**This version only** — relative to **v1.50**.  
Full history: [History of Modifications.md](./History%20of%20Modifications.md).

**Supersedes v1.50.** Active code: `HIMA-Prooftest-Solution-Current`.
...
| Aspect | Behaviour |
|--------|-----------|
| **Device list** | SILworX REST API when available; otherwise scan **X-OPC** (`opc_fallback`) |
...
| **G-10** | To identify the device list, device types, and updates, the solution must use the **SILworX integrated API** when available (details per Step below). **If SILworX/API is not available** (closed, suspended, unreachable, or uninstall), the solution **must update the device list by scanning X-OPC servers** for Prooftest Results devices (`opc_fallback`). Realtime values are always read from **X-OPC**. |
...
| Unified mode | Always `deployment_case=1`; OPC when API down | `step01_setup.py` + `step03` / `step07` | **Done** (v1.51 — Case 2 removed) |
